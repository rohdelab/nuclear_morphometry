function [cPLDA_directions,cPLDA_projectionsB,viz_plda] = scPCA_ModesL_ex1v2_both(uB1,labelB1,pt_wtB1,uB_ref,labelB2_ref,pt_wtB2_ref,ret_dir_num,xxT1,pT1,lT1,xxT_ref,pT_ref,lT_ref)
...
for a=1:length(PsiB_m0_1)
    [Vec,~,~]=svd(PsiPCAB{a},'econ'); Vec=real(Vec);
...
for ii=1:length(PsiB_m0_1)
    ...
    for b=2:ret_dir_numB{ii}
        ...
        [Vec,~,~]=svd(PsiPCA,'econ'); Vec=real(Vec);
