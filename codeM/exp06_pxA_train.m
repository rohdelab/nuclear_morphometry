...
%% BEGIN
for i_tag=TagVec
    clearvars -except i_tag TagVec Ndir Nfold force_run ref_use
    %% SELECT DATA
    switch i_tag
        case 0
            tag=0;
    ...
    logpth=[pwd '/RESULTS/EXP06_pxA/log'];
    dT=[]; pT=[]; lT=[];
    for a=1:length(subtag)
        ...
        load([inp_tr '/Lotp_' ref_use 'TSOF']); load([inp_patient_label_tr '/patient_label' num2str(tag) subtag{a}]);
...
%             [xxT,labT,~,~]=fn_trte_split2(dT,lT,ind,fold_tr,fold_te);
            ...
            [PLDA_directions,projPLDA_tr,viz_plda]=cPLDA_ModesL_pxA(xx_tr,lab_tr,p_wt_tr,Ndir,xxT,pT,labT,[ref_use 'TSOF'],size(u,1)/2);
