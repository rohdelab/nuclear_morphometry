...
%% BEGIN
for i_tag=TagVec
    clearvars -except i_tag TagVec Ndir Nfold force_run 
    %% SELECT DATA
    switch i_tag
        case 0
            tag=0;
    ...
    logpth=[pwd '/RESULTS/EXP05_pxA/log'];
    
    %  %
    
    inT=[pwd '/DATA/data' num2str(tag) 'T' '/lotp'];
    load([inT '/Lotp_' 'TSOF']);
    ...
    for a=1:length(subtag)
        ...
        load([inp_patient_label_tr '/patient_label' num2str(tag) subtag{a}]);
        patient_label_tr{a}=label_patient;
        
        en=en+length(label_patient);
        data_tr{a}=dT(:,st:en); p_wt_tr{a}=pT; label_tr{a}=lT(st:en); 
        st=1+en;
    end
    ...
    for foldset=1:Nfold
        ...
        if force_run|ISthere1==0|ISthere2==0|ISthere3==0
            ...
            [PLDA_directions,projPLDA_tr,viz_plda]=cPLDA_ModesL_pxA(xx_tr,lab_tr,p_wt_tr,Ndir,xxT,pT,labT,'TSOF',size(u,1)/2);
