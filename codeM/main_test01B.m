...
for i_tag=TagVec
    ...
    switch i_tag
        ...
        case 4
            ...
            nme=['Dataset_' num2str(tag)];
            ote={'4a','4a'}; otnme={'meso_can_reac','meso_can_reac'};
    ...
    for foldset=1:Nfold
        ...
        if ISthere1==0|ISthere2==0|ISthere3==0
            ...
            disp([Exp_str ' test01B on Data-' num2str(tag) '__: Calculating and saving... Fold ' num2str(foldset) ' of ' num2str(Nfold)]); disp(' ');
            ...
            for a=1:length(ote)
                ...
                [projPLDA_te,axis_image,dm,dt,dt_tr]=proj_n_calc_PLDAl(xx_te,lab_te,PLDA_directions{a},viz_plda{a},projPLDA_tr{a},lab_tr{a});
                DMB(1:length(dm(:)),a,foldset)=dm(:);
                DTB(a,foldset)=dt;
                DT_trB(a,foldset)=dt_tr;
                save(vnm4pnm,'projPLDA_te','-v7.3');
                axis_imageB{a}=axis_image;
            end

            for a=1:length(ote)
                axis_image=axis_imageB{a};
                f01=figure('position',[0 0 400*size(axis_image,1)/size(axis_image,2) 400]);
                ax1=axes; imagesc(flipud(axis_image'));
                xticks(linspace(1+(length(axis_image)/(2*size(PLDA_directions{a},2))),length(axis_image)-(length(axis_image)/(2*size(PLDA_directions{a},2))),size(PLDA_directions{a},2)));
                xticklabels(1:size(PLDA_directions{a},2)); yticks([]);
                ...
                set(ax1,'fontsize',15);
                title(['Single model$^{~(' num2str(foldset) '~of~' num2str(Nfold) ')}$'],'interpreter','latex','fontsize',18);

                if exist([respth '/' nme '/test01B' str_save_dir '/'])
                else
                    mkdir([respth '/' nme '/test01B' str_save_dir '/'])
                end

                saveas(f01,[respth '/' nme '/test01B' str_save_dir '/axis_set' num2str(foldset) '_' otnme{a} '_' reg_str '.png']);
            ...
            set(gca,'fontsize',15);
            title(['Mean deviation - single model'],'interpreter','latex','fontsize',18);
            subplot 122; hold on;
            for a=1:size(DMB,2)
            ...
            saveas(f02,[respth '/' nme '/test01B' str_save_dir '/meanMig_set' num2str(foldset) '_' reg_str '.png']);
            ...
            title(['Wasserstein distance - single model'],'interpreter','latex','fontsize',18);

            saveas(f03,[respth '/' nme '/test01B' str_save_dir '/wassDist_set' num2str(foldset) '_' reg_str '.png']);
    ...
    csvwrite([respth '/' nme '/test01B' str_save_dir '/' str],DD)
    d=sprintf(['\nFinished ' Exp_str '-test01B on Data' num2str(tag) '__: projected test data on PLDA directions in LOTP space; saved PLDA projections; saved the plots of the migration of the mean and axis visualization figures; saved the numerical results in excel.\n\n']); disp(d);
end
%%
