...
%% BEGIN
for i_tag=TagVec
    clearvars -except i_tag TagVec Ndir Nfold which_axis ref_use
    %% SELECT DATA
    switch i_tag
        case 0
            tag=0;
    ...
    for foldset=1:Nfold
        ...
        vnm5=[Exp_str '_data' num2str(tag) subtag{1} subtag{2} '_reg' 'TOFnTSOF' '___fold' num2str(foldset) 'of' num2str(Nfold) '_retdir' num2str(Ndir) '_pat_hist_mean_class_tr'];
...
%             disp('Patient classification (hist mean feature based); Test accuracy:')
            ...
            for a=1:length(ote)
                ...
                load([inp_te '/Lotp_' ref_use 'TOF']); load([inp_patient_label_te '/patient_label' num2str(tag) subtag{a}]);
                ...
                if exist([respth '/' nme '/testA/'])
                else
                    mkdir([respth '/' nme '/testA/'])
                end
                
                saveas(f01,[respth '/' nme '/testA/f1_' otnme{a} '_ax' num2str(which_axis(1)) '_set' num2str(foldset) '.png']);
                saveas(f02,[respth '/' nme '/testA/f2_' otnme{a} '_ax' num2str(which_axis(1)) '_' num2str(which_axis(2)) '_set' num2str(foldset) '.png']);
                pause(10); close all;
            end
            disp(' ');
        end
