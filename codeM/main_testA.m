...
for i_tag=TagVec
    ...
    for foldset=1:Nfold
        ...
        if ISthere1==0|ISthere2==0|ISthere3==0
            ...
            disp([Exp_str ' testA on Data-' num2str(tag) '__: Calculating and saving... Fold ' num2str(foldset) ' of ' num2str(Nfold)]); disp(' ');
            ...
            for a=1:length(ote)
                ...
                if reg_str_both==1
                    tmp_nm1=['Feat1_test' '_data' ote{a} '_reg' 'BOTH' '___fold' num2str(foldset) 'of' num2str(Nfold) '_retdir' num2str(Ndir)];
                    tmp_nm2=['Feat2_test' '_data' ote{a} '_reg' 'BOTH' '___fold' num2str(foldset) 'of' num2str(Nfold) '_retdir' num2str(Ndir)];
                    tmp_nm3=['Feat3_test' '_data' ote{a} '_reg' 'BOTH' '___fold' num2str(foldset) 'of' num2str(Nfold) '_retdir' num2str(Ndir)];
                else
    ...
    close(wh); close all;
    d=sprintf(['\nFinished ' Exp_str '-testA on Data' num2str(tag) '__: projected test data on PLDA directions in LOTP space; saved PLDA projections, and 1D 2D plots.\n\n']); disp(d);
end
%%
