function [ out ] = size_direction( train_tag, reg_str, Nms, I0_seed )
...
if reg_str(1)=='r'&reg_str(2)=='0'&reg_str(3)=='_'
    [X,Y]=meshgrid(linspace(-5,5,300),linspace(-5,5,300));
    msk=double(X.^2+Y.^2<1.5);
    tmp=gaussian1d(linspace(-5,5,300),1,0); tmp=tmp(:);
    I0=tmp*tmp'; I0=I0.*msk;
    I0=I0/sum(vec(I0));
else
    inpI0=[p1 '/DATA/data' num2str(train_tag) 'T'];
    cd([inpI0 '/image'])
