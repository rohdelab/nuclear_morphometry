...
%% BEGIN
for i_tag=TagVec
    clearvars -except i_tag TagVec Ndir Nfold  ref_use
    %% SELECT DATA
    switch i_tag
        case 0
            tag=0;
    ...
    respth=[pwd '/RESULTS/EXP06_pxA'];
    Exp_str = 'Exp06_pxA'; Exp_strF = '/EXP06_pxA/';
    cd(p0);
    ...
    for foldset=1:Nfold
        vnm1=[Exp_str '_data' num2str(tag) subtag{1} subtag{2} '_reg' 'TOFnTSOF' '___fold' num2str(foldset) 'of' num2str(Nfold) '_retdir' num2str(Ndir) '_PLDA_directions'];
        vnm2=[Exp_str '_data' num2str(tag) subtag{1} subtag{2} '_reg' 'TOFnTSOF' '___fold' num2str(foldset) 'of' num2str(Nfold) '_retdir' num2str(Ndir) '_projPLDA_tr'];
        vnm3=[Exp_str '_data' num2str(tag) subtag{1} subtag{2} '_reg' 'TOFnTSOF' '___fold' num2str(foldset) 'of' num2str(Nfold) '_retdir' num2str(Ndir) '_viz_plda'];
        ...
        if ISthere1==0|ISthere2==0|ISthere3==0
            ...
            load(vnm2pnm);
            clear DMB DTB DT_trB % % check this line
            for a=1:length(ote)
                vnm4=[Exp_str '_data' num2str(tag) subtag{1} subtag{2} ote{a} '_reg' 'TOFnTSOF' '_fold' num2str(foldset) 'of' num2str(Nfold) '_retdir' num2str(Ndir) '_projPLDA_te'];
            ...
            if exist([respth '/' nme '/testB/'])
            else
                mkdir([respth '/' nme '/testB/'])
            end
            
            debug_stop=4; % delete this line later
            
            saveas(f01,[respth '/' nme '/testB/axis_set' num2str(foldset) '.png']);
            ...
            saveas(f02,[respth '/' nme '/testB/meanMig_set' num2str(foldset) '.png']);
            ...
            saveas(f03,[respth '/' nme '/testB/wassDist_set' num2str(foldset) '.png']);
    ...
    d=sprintf(['\nFinished ' Exp_str '-testB on Data' num2str(tag) subtag{1} subtag{2} subtag{3} subtag{4} '__: projected test data on PLDA directions in LOTP space; saved PLDA projections; saved the plots of the migration of the mean and axis visualization figures; saved the numerical results in excel.\n\n']); disp(d);
end
%%
