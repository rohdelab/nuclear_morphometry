...
%% BEGIN
for i_tag=TagVec
    clearvars -except i_tag TagVec Ndir Nfold reg_str reg_str_both pca_or_plda_in_01 newNdir
    %% SELECT DATA
    switch i_tag
        case 0
            tag=0;
        ...
        case 1
            ...
            nme=['Dataset_' num2str(tag)];
            ote={'1a','1b','1c','1d'}; otnme={'Liver data','Thyroid data','Mesothelioma data','Melanoma data'};
        otherwise
            disp('Terminating...')
    end
    %%
    ...
    for foldset=1:1 % Nfold
        ...
        if ISthere1==0|ISthere2==0|ISthere3==0
            ...
            for a=1:length(ote)
                ...
                [projPLDA_te,axis_image,dm,dt,dt_tr]=proj_n_calc_PLDAl(xx_te,lab_te,PLDA_directions{a}(:,1:newNdir),viz_plda{a}(:,:,1:newNdir),projPLDA_tr{a},lab_tr{a});
            ...
            for a=1:length(ote)
                ...
                ytc1=[];
                for ii=1:mvim
                    if ii==floor(mvim/2)+1
                        ytc1{ii}=[num2str(ytc(ii))];
                    else
                        ytc1{ii}=[num2str(ytc(ii)) '\sigma'];
                    end
                end

                f01=figure('position',[0 0 400 350*size(axis_image,1)/size(axis_image,2)]*1.1); movegui(f01,'north')
                subplot 335
                ax1=axes;
                %                 imagesc((axis_image));
                ...
                sdir=4;
                %                 yticks(linspace(1+(length(axis_image)/(2*size(PLDA_directions{a},2))),length(axis_image)-(length(axis_image)/(2*size(PLDA_directions{a},2))),size(PLDA_directions{a},2)));
                %                 yticklabels(1:size(PLDA_directions{a},2));
                yticks(linspace(size(axis_image,1)/sdir,size(axis_image,1),sdir)-size(axis_image,1)/(2*sdir));
                yticklabels(1:sdir);
                xticks(ytc0); xticklabels(ytc1); xlabel('(\sigma)');
                colormap(ax1,'gray');  ylabel('Modes of variations','interpreter','latex');
                set(ax1,'fontsize',16);

                title([otnme{a}],'interpreter','latex','fontsize',18);

                set(gcf,'position',[0 0 400 550]); movegui(f01,'north')

                Fgflnm=['Fig_' str_save_dir(2:end) '/'];
                if exist(Fgflnm)
                else
                    mkdir(Fgflnm)
                end
                saveas(f01,[Fgflnm otnme{a} '.svg']);
                saveas(f01,[Fgflnm otnme{a} '.png']);
